...
if len(sys.argv) != 4:
    print('Arguments: config, last, batch in {2, ..., nb_classes//B}')
    sys.exit(-1)
...
epochs = int(cp['epochs'])//5

batch_size = int(cp['batch_size'])//2
lr = float(cp['lr'])
...
# verify that the batch is at least 2 and at most nb_classes//B
assert nb_batch_to_compute >= 2 and nb_batch_to_compute <= nb_classes//first_batch_size
B = first_batch_size
...
if not os.path.exists(os.path.join(output_dir,last,f'batch{nb_batch_to_compute}.pth')):
    ...
    model.fc = nn.Linear(model.fc.in_features, nb_batch_to_compute*B)
    if last=='last':
        compteur_limit = 57
    elif last=='half':
        compteur_limit = 54
    ...
    for name, param in model.named_parameters():
        ...
        if compteur<=compteur_limit:
            ...
            if compteur==1:
                print('on freeze',name, compteur)
                print('...')
            if compteur>=compteur_limit-4:
                print('on freeze',name, compteur)
        else:
            print('on ft',name)
    ...
    val_dataset = ImagesListFileFolder(
                ...
                transforms.Compose([
                    ...
                    transforms.ToTensor(),
                    normalize,
                ]), random_seed=random_seed, range_classes=range((-1+nb_batch_to_compute)*B,(nb_batch_to_compute)*B), nb_classes=nb_classes)
    ...
    ckp_name = os.path.join(output_dir,last,f'batch{nb_batch_to_compute}.pth')
